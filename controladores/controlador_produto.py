...
class ControladorProduto:
    ...
    def seleciona_produto(self, produtos):
        ...
        for p in produtos:
            try:
                nome = p.nome
                info = "Nome: {} | Descricao:  {} | Categoria: {} | ".format(
                    nome, p.descricao, p.categoria.nome
                )
                dados[str(p.id)] = info
            except:
                continue
        ...
        for p in produtos:
            produtos_ordenados.append(
                [
                    ...
                    p.descricao,
                    p.categoria.nome,
                    p.precos[0].valor if p.precos else "Sem Preco",
                    p.precos[0].contador if p.precos else 0,
                    self.formata_data(p.data_criacao),
                ]
            )
        ...
        for p in produtos:
            produtos_ordenados.append(
                [
                    ...
                    p.descricao,
                    p.categoria.nome,
                    p.precos[0] if p.precos else "Sem Preco",
                    p.precos[0] if p.precos else 0,
                    self.formata_data(p.data_criacao),
                ]
            )
        return produtos_ordenados
        ...
        for preco in precos:
            dados.append(
                [
                    ...
                    preco.valor,
                    preco.contador,
                    self.formata_data(preco.produto.data_criacao),
                ]
            )
        return dados
    ...
    def cadastra_produto(self):
        if not self.verifica_existe_mercado():
            ...
            return
        dados = self.tela_produto.pega_dados_produto(
            [c.nome for c in self.controlador_sessao.controlador_categoria.categorias]
        )
        if not dados:
            return
        ...
        if not mercado:
            return
        produto_duplicado = self.verifica_produto_duplicado(
        ...
        if produto_duplicado:
            ...
            return
        categoria = self.controlador_sessao.controlador_categoria.get_categoria(
            dados["categoria"]
        )
    ...
    def adicionar_preco_produto(self):
        ...
        if preco:
            preco.contador += 1
            self.controlador_sessao.controlador_preco.preco_DAO.update(preco)
            self.controlador_sessao.controlador_produto.produto_DAO.update(
                preco.produto
            )
    ...
    def busca_produto(self):
        ...
        if not dados_busca:
            return
        produtos_filtrados = self.filtra_produtos(
            dados_busca["nome_produto"], dados_busca["categoria"]
        )
        if not produtos_filtrados:
            self.tela_produto.mostra_mensagem("Nenhum Produto Encontrado")
            return
        dados = self.ordena_produtos(produtos_filtrados, dados_busca["modo"])
        self.tela_produto.mostra_dados_produtos(
            dados, additional_header="DATA PUBLICACAO"
        )
    ...
    def remove_qualificador_produto(self):
        ...
        id_qualificador = (
            self.controlador_sessao.controlador_qualificador.pega_qualificador(dados)
        )
