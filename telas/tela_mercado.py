...
class TelaMercado(TelaAbstrata):
    ...
    def pega_dados_mercado(self, cnpj=True, permitir_vazio=False):
        ...
        while True:
            event, values = self.__window.read()
            if event == "cadastrar":
                if not all([bool(v) for k, v in values.items()]):
    ...
    def seleciona_mercado(self, dados):
        l = [[sg.Text("SELECIONE UM DOS MERCADOS ABAIXO")]]
        for id, dado in dados.items():
            l.append([sg.Radio(text=dado, key=id, group_id="")])
        l.append(
            [
                sg.Button("Selecionar Mercado", key="selecionado"),
    ...
    def edita_mercado(self, default_data):
        layout = [
            ...
            [
                ...
                sg.Input(key="cep", default_text=default_data["cep"]),
            ],
            [
                sg.Text("NUMERO"),
                sg.Input(key="numero", default_text=default_data["numero"]),
            ],
        ...
        while True:
            event, values = self.__window.read()
            if event == "editar":
                if any([v == "" for k, v in values.items()]):
